use stm32f4xx_hal::gpio::{PA5, PA6, PA7};
...
impl<const PC: char, const NC: u8> MAX31865<PC, NC> {
    pub fn new(mut cs_pin: Pin<PC, NC, Output>, offset: f32, wires: u8) -> Self {
        cs_pin.set_high();
        MAX31865 {
            cs_pin,
            offset,
    ...
    pub fn init(&mut self, spi: &mut Spi<SPI1, (PA5, PA6, PA7)>) -> Result<u8, Error> {
    ...
    pub fn get_raw_resistance(&mut self, spi: &mut Spi<SPI1, (PA5, PA6, PA7)>) -> Option<u16> {
    ...
    pub fn get_resistance(&mut self, spi: &mut Spi<SPI1, (PA5, PA6, PA7)>) -> Option<f32> {
        ...
        if let Some(raw_resistance) = self.get_raw_resistance(spi) {
            Some(
                (raw_resistance as f32 * MAX31865_RREF as f32) / MAX31865_FACTOR as f32
                    - self.offset,
