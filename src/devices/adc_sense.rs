...
pub static G_PRESSURE_VOLTAGE: Mutex<RefCell<Option<f32>>> = Mutex::new(RefCell::new(None));
pub static G_CURRENT_VOLTAGE: Mutex<RefCell<Option<f32>>> = Mutex::new(RefCell::new(None));
...
pub fn read_pressure() -> Option<f32> {
    let mut voltage = 0.0;
    cortex_m::interrupt::free(|cs| match G_PRESSURE_VOLTAGE.borrow(cs).borrow_mut().as_mut() {
        None => {}
    ...
    // https://www.amazon.de/SS316L-Drucktransmitter-Maschine-Druckwandler-Sensor/dp/B0DPXCL714?th=1
    // https://de.aliexpress.com/item/1005005333136580.html
    // 0.5 - 4.5 V output for 0 - 20 bar, linear, so we can just do a simple linear transformation
    // p = (v - 0.5) / 4.0 * 20.0

    // usb_println(arrform!(64, "Pressure Voltage: {:}", voltage).as_str());

    if (0.25..=5.0).contains(&voltage) {
        Some((voltage - 0.5) * 1.0 / 4.0 * 20.0)
    } else {
        None
    }
}

pub fn read_current() -> Option<f32> {
    let mut voltage = 0.0;
    cortex_m::interrupt::free(|cs| match G_CURRENT_VOLTAGE.borrow(cs).borrow_mut().as_mut() {
        None => {}
        Some(sampled_voltage) => voltage = *sampled_voltage,
    });

    if (0.0..=3.3).contains(&voltage) {
        Some(voltage / 3.3 * 10.0)
    } else {
        None
    }
}
        ...
        if let Some(xfer) = G_XFR.borrow(cs).borrow_mut().as_mut() {
            unsafe {
                if let Ok((buffer, _)) = xfer.next_transfer(&mut ADC_MEMORY) {
                    ...
                    G_PRESSURE_VOLTAGE.borrow(cs).replace(Some(
                        sample_to_millivolts(buffer[1]) as f32 / 1000.0 - OFFSET,
                    ));
                    G_CURRENT_VOLTAGE
                        .borrow(cs)
                        .replace(Some(sample_to_millivolts(buffer[2]) as f32 / 1000.0));
